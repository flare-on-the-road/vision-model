...
GTX 1060은 Pascal 세대 GPU입니다. 최신 `onnxruntime-gpu[cuda,cudnn]`가 설치하는 CUDA 12 + cuDNN 9 조합에서는 `no kernel image is available for execution on the device` 오류가 날 수 있습니다. 1060에서는 Python 3.10 또는 3.11 가상환경과 CUDA 11.8/cuDNN 8 계열을 사용합니다.
...
`libcublasLt.so.12` 오류는 CUDA 12 런타임이 없는 상태입니다. `no kernel image is available for execution on the device` 오류는 최신 cuDNN/CUDA 커널이 GTX 1060에 맞지 않는 상태입니다. 둘 다 기존 가상환경을 지우고 Python 3.10/3.11 + `onnxruntime-gpu==1.18.0` 조합으로 다시 만드는 쪽이 가장 단순합니다.
